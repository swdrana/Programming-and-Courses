...
import i from "../../assets/book.png";
import starIcon from "../../assets/star.svg";
function Book({ book, handelFeb }) {
  const { id, title, author, year, price, star, isFavourite, img } = book;

  return (
    <div className="space-y-3">
      {/* <!-- thumbnail --> */}
      <div className="flex items-center justify-center rounded-md border border-[#324251]/30 bg-white p-4">
        <img className="max-w-[144px]" src={i} alt="book name" />
      </div>
      {/* <!-- info --> */}
      <div className="space-y-3">
          ...
          <button
            className={`flex min-w-[132px] items-center justify-center gap-1 rounded-md  py-1.5 ${
              isFavourite
                ...
                : "text-[#1C4336] bg-[#1C4336]/[14%] hover:bg-[#1C4336]/[24%]"
            } transition-all lg:py-1.5`}
            onClick={()=>handelFeb(id)}
          >
            {isFavourite ? (
              <svg
                width="16"
